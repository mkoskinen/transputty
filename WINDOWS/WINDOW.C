...
/* robymus background patch */
#define IDM_BACKGROUND	0x0790
...
/* robymus backgroud patch */
static int backgroundenabled = 0;
static HBITMAP backgroundbmp;
static HDC backgroundhdc;
...
/*
 * robymus background patch - setenablebackground
 */
void setenablebackground(int newstate)
{
	HMENU m;
	if (backgroundenabled == newstate) return;
	if (newstate) {
		/* enable - load bitmap */
		backgroundbmp = LoadImage(hinst, backgroundname, IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE);
		if (!backgroundbmp) return;
		backgroundhdc = CreateCompatibleDC(NULL);
	...
	if (newstate) {
		CheckMenuItem(m, IDM_BACKGROUND, MF_BYCOMMAND | MF_CHECKED);
	}
	else {
		CheckMenuItem(m, IDM_BACKGROUND, MF_BYCOMMAND | MF_UNCHECKED);
	}

	backgroundenabled = newstate;
...
void background_paint(Context hdc, int x, int y, int xs, int ys)
{
	    POINT p;
	    p.x = x;
	    p.y = y;
	    ClientToScreen(hwnd, &p);
	    BitBlt(hdc, x, y, xs, ys, backgroundhdc , p.x, p.y, SRCCOPY);
}

void background_fixborders(Context ctx, int cols, int rows)
{
    int vertres = GetDeviceCaps(ctx, VERTRES);
    int horzres = GetDeviceCaps(ctx, HORZRES);
    int remain = vertres - rows * font_height;
    if (remain) {
	background_paint(ctx, 0, vertres-remain, horzres, remain);
    }
    remain = horzres - cols * font_width;
    if (remain) {
	background_paint(ctx, horzres - remain, 0, remain, vertres);
    }
}
...
int WINAPI WinMain(HINSTANCE inst, HINSTANCE prev, LPSTR cmdline, int show)
{
     ...
     */
    /* robymus background patch parse filename */
    backgroundname = strdup("putty.bmp");
	...
	for (j = 0; j < lenof(popup_menus); j++) {
		...
		/* !! robymus background patch !! */
		AppendMenu (m, MF_SEPARATOR, 0, 0);
		AppendMenu (m, MF_ENABLED, IDM_BACKGROUND, "&Background");
		setenablebackground(1);
	    sfree(str);
	}
    }
...
void cleanup_exit(int code)
{
	...
	/* robymus background patch : cleanup background object */
    if (backgroundenabled) setenablebackground(0); 
    free(backgroundname);
			  ...
			  CONST INT *lpDx, int opaque)
{
	...
	/* robymus transparency (edited opaque) */
    ExtTextOut(hdc, x, y,
	       ETO_GLYPH_INDEX | ETO_CLIPPED | ((!backgroundenabled && opaque) ? ETO_OPAQUE : 0),
			    ...
			    CONST INT *lpDx, int opaque)
{
	...
	if (rtl) {
		...
		/* robymus background */
	    ExtTextOutW(hdc, xp, y, ETO_CLIPPED | ((!backgroundenabled && opaque) ? ETO_OPAQUE : 0),
			lprc, lpString+i, j-i,
				...
				WPARAM wParam, LPARAM lParam)
{
	...
	switch (wParam & ~0xF) {       /* low 4 bits reserved to Windows */
	  ...
	  /* robymus background patch */
	  case IDM_BACKGROUND:
	    setenablebackground(!backgroundenabled);
	    break; 
	  case IDM_HELP:
		      ...
		      unsigned long attr, int lattr)
{
	...
	if (len > lpDx_len) {
        ...
        if (ucsdata.dbcs_screenfont && (text[0] & CSET_MASK) == CSET_ACP) {
			...
			if (backgroundenabled && (bg == RGB(0,0,0))) {
			    background_paint(hdc, x, y, fnt_width*len, font_height);
			...
			/* !! patch here if nbg = black, then use background - and backgroundenabled !! - robymus */
			//SelectObject (hdc, fonts[nfont]);
			//SetTextColor (hdc, fg);
